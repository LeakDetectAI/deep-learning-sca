...
class AESRDAttack(AttackModel):
    def __init__(self, model_name, model_class, loss_name, num_attacks, total_attack_traces, real_key, byte,
        ...
        # Logger
        self.logger = logging.getLogger(AESRDAttack.__name__)
        super().__init__(AES_SBOX=AES_RD_Sbox, model_name=model_name, model_class=model_class, loss_name=loss_name,
    ...
    def _store_results(self, dataset=AES_RD):
        super()._store_results(dataset=dataset)

    def _load_attack_model_(self, dataset=AES_RD):
