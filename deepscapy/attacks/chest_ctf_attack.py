...
class CHESCTFAttack(AttackModel):
    def __init__(self, model_name, model_class, loss_name, num_attacks, total_attack_traces, dataset_type,
        ...
        # Logger
        self.logger = logging.getLogger(CHESCTFAttack.__name__)
    ...
    def _rank_compute(self, prediction, plaintext, secret_key, byte):
        ...
        for i in range(nb_trs):
            for k in range(nb_hyp):
                idx = self.AES_SBOX[int(k) ^ int(plaintext[i, byte])]
                # self.logger.info(f"Trace {i} Key Byte {k} Index {idx} Plaintext Byte {int(plaintext[i, byte])}")
                if self.leakage_model == HW:
                    idx = self.hw_box[idx]
                key_log_prob[k] += prediction[i, idx]
            # self.AES_SBOX[key[byte] ^ plaintext[i, byte] # TODO to generate the label for CHES, 3rd byte
            rank_evol[i] = self._rk_key(key_log_prob, int(secret_key[byte]))
    ...
    def _store_results(self, dataset=CHES_CTF):
        super()._store_results(dataset=dataset)

    def _load_attack_model_(self, dataset=CHES_CTF):
