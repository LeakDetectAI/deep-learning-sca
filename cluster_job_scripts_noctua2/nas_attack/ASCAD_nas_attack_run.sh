...
export MODEL_NAME=$3
export TUNER=$4
export RESHAPE_TYPE=$5

module list
singularity exec -B $PFS_FOLDER/deep-learning-sca/ --nv $IMG_FILE pipenv run python $SCRIPT_FILE --dataset=$DATASET --loss_function=$LOSS_FN --model_name=$MODEL_NAME --tuner_type=$TUNER --reshape_type=$RESHAPE_TYPE --no-use_tuner
