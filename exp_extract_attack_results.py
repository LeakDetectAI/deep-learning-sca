...
if __name__ == "__main__":
    ...
    min_complete_mean_rank_df = pd.DataFrame(columns=loss_name_list_ext, index=model_name_index)
    last_index_mean_rank_df = pd.DataFrame(columns=loss_name_list_ext, index=model_name_index)
    min_last_100_mean_rank_df = pd.DataFrame(columns=loss_name_list_ext, index=model_name_index)
            ...
            if file.endswith('.h5'):
                ...
                with h5py.File(file_path, 'r') as hdf:
                    ...
                    if not ('_tuned' in model_folder_name or 'weight_averaging' in model_folder_name or 'lenet' in model_folder_name):
                        lossse = loss + SE
                        trainable_params = np.array(hdf['model_parameters/{}'.format(TRAINABLE_PARAMS)])
                        non_trainable_params = np.array(hdf['model_parameters/{}'.format(NON_TRAINABLE_PARAMS)])
                        total_params = np.array(hdf['model_parameters/{}'.format(TOTAL_PARAMS)])

                        logger.info('Trainable params = {}'.format(trainable_params))
                        logger.info('Non-trainable params = {}'.format(non_trainable_params))
                        logger.info('Total params = {}'.format(total_params))

                        mean_rank_final_df.at[model_name, loss] = np.around(np.mean(mean_rank_final), 2)
                        mean_rank_final_df.at[model_name, lossse] = np.around(np.std(mean_rank_final) / math.sqrt(10), 2)

                        min_complete_mean_rank_df.at[model_name, loss] = np.around(np.mean(min_complete_mean_rank), 2)
                        min_complete_mean_rank_df.at[model_name, lossse] = np.around(np.std(min_complete_mean_rank) / math.sqrt(10), 2)

                        last_index_mean_rank_df.at[model_name, loss] = np.around(np.mean(last_index_mean_rank), 2)
                        last_index_mean_rank_df.at[model_name, lossse] = np.around(np.std(last_index_mean_rank) / math.sqrt(10), 2)

                        min_last_100_mean_rank_df.at[model_name, loss] = np.around(np.mean(min_last_100_mean_rank), 2)
                        min_last_100_mean_rank_df.at[model_name, lossse] = np.around(np.std(min_last_100_mean_rank) / math.sqrt(10), 2)
                        ...
                        number_of_traces_df.at[model_name, lossse] = np.around(np.nanstd(number_of_traces) / math.sqrt(10), 6)
    ...
    mean_rank_final_df.to_excel(os.path.join(attack_results_path, 'mean_rank_final_{}_attack.xlsx'.format(dataset)),
                                index=True, header=True, index_label='Model Name')
    min_complete_mean_rank_df.to_excel(
        os.path.join(attack_results_path, 'min_complete_mean_rank_{}_attack.xlsx'.format(dataset)),
        index=True, header=True, index_label='Model Name')
    last_index_mean_rank_df.to_excel(
        os.path.join(attack_results_path, 'last_index_mean_rank_{}_attack.xlsx'.format(dataset)),
        index=True, header=True, index_label='Model Name')
    min_last_100_mean_rank_df.to_excel(
        os.path.join(attack_results_path, 'min_last_100_mean_rank_{}_attack.xlsx'.format(dataset)),
        index=True, header=True, index_label='Model Name')
    accuracy_df.to_excel(os.path.join(attack_results_path, 'accuracy_{}_attack.xlsx'.format(dataset)), index=True,
    ...
    trainable_params_df.to_excel(os.path.join(attack_results_path, 'trainable_params_{}_attack.xlsx'.format(dataset)),
                                 index=True, header=True, index_label='Model Name')
    non_trainable_params_df.to_excel(os.path.join(attack_results_path, 'non_trainable_params_{}_attack.xlsx'.format(dataset)),
                                     index=True, header=True, index_label='Model Name')
    total_params_df.to_excel(os.path.join(attack_results_path, 'total_params_{}_attack.xlsx'.format(dataset)),
                             index=True, header=True, index_label='Model Name')
